...
{
    return ((u64(Utils::FloatToFixedPoint<u16>(yOffset) & 0xFFFF) << 32) |
            u64(Utils::FloatToFixedPoint<u16>(xOffset) & 0xFFFF));
}

u64 DrawingEnvironment::GetScissoringAreaSettings() const
{
    return ((u64((framebuffer[0]->GetHeight() - 1) & 0x7FF) << 48) | (u64(0 & 0x7FF) << 32) |
            (u64((framebuffer[0]->GetWidth() - 1) & 0x7FF) << 16) | u64(0 & 0x7FF));
}

u64 DrawingEnvironment::GetAlphaAndDepthTestSettings() const
{
    return (static_cast<u64>(zbuffer->GetDepthTestMethod()) & 0x03) << 17 | u64(0x01) << 16 |
           alphaTest.GetAlphaTestSettings();
}

u64 DrawingEnvironment::GetDisabledAlphaAndDepthTestSettings() const
{
    return (static_cast<u64>(ZbufferTestMethod::ALLPASS) & 0x03) << 17 | u64(0x01) << 16 |
           alphaTest.GetAlphaTestSettings();
}
...
{
    return u64(0x80 & 0xFF) << 32 | (static_cast<u64>(AlphaBlendingColorConfig::COLOR_DESTINATION) & 0x03) << 6 |
...
u64 DrawingEnvironment::GetTextureWrappingSettings(TextureWrappingOptions wrapOptions,
                                                   unsigned int minU,
                                                   unsigned int maxU,
                                                   unsigned int minV,
                                                   unsigned int maxV) const
{
    return ((static_cast<u64>(maxV) & 0x3FF) << 34) | ((static_cast<u64>(minV) & 0x3FF) << 24) |
           ((static_cast<u64>(maxU) & 0x3FF) << 14) | ((static_cast<u64>(minU) & 0x3FF) << 4) |
           ((static_cast<u64>(wrapOptions) & 0x03) << 2) | (static_cast<u64>(wrapOptions) & 0x03);
}

...
{
    address =
        graph_vram_allocate(this->width, this->height, static_cast<int>(this->pixelStorageMethod), GRAPH_ALIGN_PAGE);
    return address;
}
...
{
    return (u64(mask) << 32 | (static_cast<u64>(pixelStorageMethod) & 0x3F) << 24 | u64((width >> 6) & 0x3F) << 16 |
